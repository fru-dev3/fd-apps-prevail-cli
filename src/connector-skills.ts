...
export async function runSkill(
  ...
  // NOTE on the cli runner: it executes an arbitrary `/bin/sh -c` command
  // TEMPLATE (untrusted substituted VALUES are shellQuote'd, but the template
  // itself is skill-author-controlled). That template is not confined by Vault
  // Lock and not scanned by the egress guard, so a model-authored skill could in
  // principle exfiltrate. A blanket block here breaks legitimate user-installed
  // cli connectors (a supported pattern), so the real mitigation is OS-level
  // sandboxing of skill execution (tracked with the act-run Bash sandbox), not a
  // functionality-removing gate. The http/a2a runners DO egress-scan their
  // outbound bodies (see runners.ts).
